"""Run one final non-outcome Study 1B S2 reference-SD confirmation shard.

This executor is deliberately separate from the S2 pilot executor. It can only use a
candidate that survived the predeclared pilot and it is capped by the contract's
final-datasets-per-cell budget. It never opens scientific outcomes or replaces the
canonical failed power gate.
"""
...
def main() -> int:
    ...
    parser.add_argument("--candidate-rule", required=True)
    ...
    allowed_rules = {item["id"]: int(item["minimum_seed_passes"]) for item in s2["decision_rules"]}
    final_budget = int(s2["final_datasets_per_cell_if_candidate_survives_pilot"])
    ...
    if args.candidate_rule not in allowed_rules:
        raise ValueError("candidate rule is not predeclared in S2 contract")
    if args.candidate_rule != "at_least_3_of_5":
        raise ValueError("only the sole S2-pilot surviving simple rule may enter final confirmation")
    if args.start < 0 or args.stop <= args.start:
        raise ValueError("invalid shard range")
    if args.stop > final_budget:
        raise ValueError("S2 final shard exceeds predeclared final budget")

    rows = load_subject_graph(args.test_graph)
    scenario = scenario_for_graph(
        f"s2_final_power_delta_{args.effect:g}_sd_{args.seed_effect_sd:g}",
        args.effect,
        rows,
    ...
    for dataset_index in range(args.start, args.stop):
        ...
        result.update(
            {
                ...
                "execution_role": "FINAL_REFERENCE_SD_CONFIRMATION",
        ...
        print(
            f"[S2-final] sd={args.seed_effect_sd:g} delta={args.effect:g} "
