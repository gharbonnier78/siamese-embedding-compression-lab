...
class VectorizedEdgeWeightTests(unittest.TestCase):
    ...
    def test_large_subject_universe_matches_legacy_and_preserves_int64(self) -> None:
        subject_count = 1000
        rows = [
            SubjectPairRow(
                f"g_{index}",
        ...
        values = rng.multinomial(
            subject_count,
            np.full(subject_count, 1.0 / subject_count),
        ...
        expected = edge_weights(rows, multiplicities)
        actual = edge_weights_vectorized_from_array(prepared, values)
