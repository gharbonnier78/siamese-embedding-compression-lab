...
class VectorizedCoverageExecutionTests(unittest.TestCase):
    ...
    def test_vectorized_worker_four_oversubscription_preserves_exact_outputs(
        self,
    ) -> None:
        serial = run_coverage_scenario_datasets(
            self.scenario,
    ...
    def test_degenerate_outcomes_round_trip_through_process_pool(self) -> None:
        degenerate_scenario = CoverageScenario(
            name="vectorized_degenerate_pool_fixture",
            target_delta_fnmr=0.015,
            n_subjects=3,
            n_genuine=1,
            n_impostor=1,
        ...
        scenario_seed = spawn_scenario_seed_sequences(1, 1)[0]
        serial = run_coverage_scenario_datasets(
            degenerate_scenario,
            simulated_datasets=3,
        ...
        self.assertEqual(parallel, serial)
        self.assertTrue(all(outcome.degenerate for outcome in parallel))
        for outcome in parallel:
            self.assertIsNone(outcome.representation_delta_sha256)
            self.assertIsNone(outcome.operational_fnmr_sha256)
            self.assertIsNone(outcome.operational_fmr_sha256)
