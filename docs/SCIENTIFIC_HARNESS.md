...
Performance engineering may change implementation structure but must not silently change:

- sampling unit;
- random-number semantics when those are frozen;
- threshold/tie rules;
- estimands;
...
Equivalence tests are required when an implementation is optimized after a reference implementation has been reviewed.
...
The machine-readable chronicle is `protocol/scientific_chronicle.yaml`. Each entry must contain at least:
...
Research Assurance must fail if:

- the chronicle file is missing or not append-only by declared policy;
- chronicle IDs are duplicated;
- a required field is absent;
- an OPEN entry has no `next_action`;
- an entry that blocks a named execution step is silently marked INFORMATIONAL;
...
A separate execution preflight should block a production study step when an OPEN chronicle entry explicitly lists that step in `blocks`, unless a versioned resolution or accepted-risk entry exists.
...
The arrows do not imply that later layers can repair missing evidence in earlier layers. The chronicle and pedagogical artifacts explain and preserve context; they do not manufacture statistical support.

## 7. Current programme-specific rule

For Study 0 v0.2.2, the production coverage gate must not be executed while the recorded computational-feasibility risk around the nested subject-bootstrap simulation remains OPEN. The acceptable resolution is either:
...
Reducing the preregistered number of bootstrap replicates or simulation scenarios merely for runtime convenience is not a semantics-preserving optimization and would require a versioned methodological amendment.
